...
<b>Note:</b> In Linux and UNIX, you may have to run this command as [!UICONTROL sudo] -u user jps -l, where "user" is the username of the user that the Java process is running as.
...
- If the jstack command (jstack -l pid  threaddumps.log) throws the error 1 below, then run the command as the user that owns the java process.  For example:
   ...
   sudo -u [!DNL sling] jstack -l [!DNL pid]  threaddumps.log
...
If you are using [!DNL Adobe Experience Manager] product then you can install [this tool](https://helpx.adobe.com/experience-manager/kb/thread-dumps-collection-analysis.html) to have a simple UI for generating thread dumps.
...
1. Download [!DNL javadump].exe (attached below).
...
5. From the command line, run
   ...
   javadump.exe *pid*
6. The thread dump will appear in the `[!DNL jvmoutput.log]` file mentioned in step 2.
...
3. Download and extract the [[!UICONTROL Thread] Dump Analyzer utility](https://github.com/irockel/tda). 1
4. Run `jconsole.exe` of JDK 1.6:

                               `jconsole.exe -pluginpath /path/to/file/tda.jar`
5. Click the <b>[!UICONTROL Thread dumps]</b> tab.
6. Click the <b>[!UICONTROL Request Thread Dump]</b> link.
