...
In non-production environments, ACS [!UICONTROL AEM tools] package can be installed to provide an explain query tool for further debugging.
...
- Set the Boolean property saveDirectoryListing=true (only if using Oak 1.0.16, if later version then not needed)and set property [!UICONTROL excludedPaths=/var], /etc/workflow/instances, /jcr:system on the /oak:index/lucene OOTB index to optimize it.
- Click "Save All"
...
- When async indexing fails then when you check the [!UICONTROL Async Indexer] stats JMX UI (link below), you would see that LastIndexedTime is an old date and time:
