...
The `classpath` tag from the `serverConf].xml` configuration file has not been updated correctly after the upgrade. The `classpath` tag should point to the latest JARs.
