...
| --- | --- | --- |
| FILENAME.conf | /etc/httpd/conf.d/ | A default Enterprise Linux install uses this file extension and include folder as a place to override settings in [!DNL httpd].conf and allow you to add addtional functionality at a global level in Apache. |
...
| FILENAME_whitelist.rules | /etc/httpd/conf.d/whitelists/ | \*_ipwhitelist.rules files are included from inside the [!DNL \*.vhost] files. It contains IP regex or allow deny rules to allow IP whitelisting. If you're trying to restrict viewing of a virtual host based on IP addresses you'll generate one of these files and include it from your \*.vhost file |
...
<b>[!DNL /etc/httpd/conf.d/exampleco.conf]</b>
...
If the vhost file is accidentally put in the rewrites folder and the rewrites file is put into the [!DNL vhosts] folder.  It would appear to be deployed by filename properly, yet Apache will throw an error and the problem will not be immediately apparent.
...
<b>The file extensions are the same and [!UICONTROL auto-include] prone</b>
...
Name the files base on what they do and safely out of the [!DNL auto-include rules] namespace.

If it's a virtual host file name it with .vhost as the extension.

If it's a rewrite rule file, name it with [!DNL site_rewrite.rules] as the suffix and extension. This naming convention will make it clear which site it's for and that it's a set of rewrite rules.

If it's a IP whitelist rule file, name it [!DNL description_whitelist.rules] as the suffix and extension. This naming convention will give some description of what it's for and that it's a set of IP matching rules.
...
When apache loads it will pull in the <b>[!DNL /etc/httpd/conf].modules.d/02-dispatcher.conf</b> and that file will include the binary file <b>[!DNL /etc/httpd/modules/mod_dispatcher].so</b> into it's running state.
<br><br><br><br><br> <br><br><br><br>
...
<br><br><br><br><br> <br><br>
To use the module in our *VirtualHost /* we drop a configuration file into */etc/httpd/conf.d/<b><ex id="9"/> </b>named <b>dispatcher_vhost.conf</b> and inside this file you'll see use setup the basic parameters needed for the module to work:
<br><br><br><br><br> <br><br><br><br>*
...
Later in the <b>[!DNL dispatcher_vhost].conf</b> file mentioned earlier we also do an include statement to enable each enabled virtual host files that live in <b>[!DNL /etc/httpd/conf].d/enabled_vhosts/</b> with the filename of <b>[!DNL FILENAME.vhost]</b> which follows our standard naming convention.


<br><br><br><br><br> <br><br><br><br>

| 1 | `[!DNL IncludeOptional /etc/httpd/conf].d/enabled_vhosts/*.vhost` |
| --- | --- |
...
Inside the file <b>[!DNL /etc/httpd/conf].d/variables/weretail.vars</b> we can see what variables are defined:
...
<FILENAME><br><br><br><br><br> <br><br>AMS Farm Includes<br><br><br><br><br><br>![_farms.any will include sub .any files to complete a farm configuration.  In this picture you can see that a farm will include each top level section files cache, clientheaders, filters, renders, and vhosts .any files](https://helpx.adobe.com/content/dam/kb/en/ams-dispatcher-manual/Apache-Webserver-AMS-Farm-Includes.png "Apache-Webserver-AMS-Farm-Includes")<br><br><br><br> <br><br>
...
<br><br><br><br><br> <br><br>
As you can see each section for the [!UICONTROL weretail] farm instead of having all of the syntax needed it's instead using an include statement.
