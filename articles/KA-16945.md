...
2. Once the zero ID record has been deleted, what is the recommended approach to restoring it? A direct SQL command? Find out all integer columns in the schema and create an INSERT query with 0 populated for all those columns. This should recreate the 0 record. You will have to create a SQL [!UICONTROL script] in the script section of Explorer in Campaign or use a [!UICONTROL SQL code] activity in a workflow to be able to do this.
