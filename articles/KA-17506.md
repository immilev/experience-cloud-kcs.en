...
The SMPP connection starts by connecting using TCP. Then a `BIND` operation is sent by [!DNL Campaign], acknowledged by a `BIND RESP`. These operations are described in section 4.1 of the SMPP specification (`BIND` operation).
...
In [!DNL Campaign], you should see a *`BIND_TRANSMITTER`* packet when initiating an MT transfer and a *`BIND_RECEIVER`* packet when *`nlsm s`* triggers an MO/SR connection.

<b>Transmitter, receiver and transceiver: </b>The SMPP connector for Campaign Classic works in a separate transmitter/receiver mode: there are two TCP connections, one for transmitting MT and another for receiving MO and SR. Note that the TCP connection is always initiated by [!DNL Campaign], even for the receiver mode.
