...
How to Migrate Users and Groups With ACL Permissions in [!DNL AEM] from one server to another or from one [!DNL AEM] instance to another.


## Resolution


<b>Step 1: Find the admin and anonymous users</b>
...
<b>Step 2A: Migrate users and groups (using crx2oak or oak-upgrade)</b>
...
7. Then run the command on the server's shell.     `java -Xms2g -Xmx2g -jar oak-upgrade-1.8.12.jar \`
   `--include-paths=/home \`
   `--merge-paths=/home \`
...
8. Once completed, then go to step 3 to migrate the ACLs.  If you are unable to migrate using [!UICONTROL crx2oak] then follow the package migration steps below instead.


<b>Step 2B: Migrate users and groups (using packages)</b>

If users were not imported automatically via LDAP / SAML authentication or [!UICONTROL CRX2Oak] (method A above) then you can package users and groups.  In this case you create two separate packages on the old system (excluding admin and anonymous out-of-the-box users).

1. Copy the path of the [!UICONTROL anonymous] and [!UICONTROL admin] user nodes from the results in Step 1 ("Find the admin and anonymous users") above.
...
8. Open the file [!DNL META-INF/vault/filter.xml] in a text editor.
...
12. Repeat steps 11-14 for the [!UICONTROL groups] package.
...
5. Click Edit.
6. Select the [!UICONTROL Advanced] tab (see screenshot below).
7. In the AC Handling drop-down menu select [!UICONTROL Merge] to avoid removing existing ACLs on the destination system. This is especially important when migrating ACLs between different versions of AEM (as it avoids removing out of the box ACLs).


If you are *not* able to install tools (ACS Commons) to AEM, then follow these steps. Note that the machine where you run these commands must be Mac OS, Linux, or Windows (using Cygwin) with cURL, [!DNL python], and Java SDK installed.
...
8. In the same directory, run this command to download a json file of the ACL paths under /content from the source instance (set the user name and password and correct host): 

   `curl -u admin:admin 'http://aemhost/crx/de/query.jsp?' -G --data-urlencode '_dc=1507011481908&_charset_=utf-8&type=xpath&stmt=/jcr:root/content//element(*,rep:ACL)&showResults=true'  data.json`


9. Create a file generate-package-filter.py and paste the [!DNL python] code below inside it: 
...
10. Run the [!DNL python] script from the same folder where data.json was created and save the output to [!DNL META-INF/vault/filter.xml] (replacing the existing contents of [!DNL filter].xml): 
...
13. Click [!UICONTROL Build] or [!UICONTROL Rebuild] to build the package.
...
16. Click [!UICONTROL Install] to install it.
