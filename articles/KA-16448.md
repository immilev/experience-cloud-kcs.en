...
1. Download the `crx2oak` or `oak-upgrade` jar matching the [!DNL Oak] version you are using: 
...
13. (Upgrade only) If performing migration to newer AEM version, then install a fresh local AEM instance *of the old version*(with nosamplecontent), and install the users package, and then the groups package there. Then, perform an in-place upgrade to the new version on that instance. This converts the users to the new [!DNL Oak] representation. After the in-place upgrade, repackage the users again to port them to your intended upgraded instance. Do the same for the user groups.
...
4. Select the [!UICONTROL Advanced] tab and set AC Handling mode to Merge.
5. Save.
...
10. Run the [!DNL Python] script from the same folder where data.json was created and save the output to `META-INF/vault/filter.xml` (replacing the existing contents of `filter.xml`): 
