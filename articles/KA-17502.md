...
- /etc/httpd/conf.d/available_vhosts/
   - This folder will host all of your [!DNL VirtualHost] / files called .vhost
...
- *[!DNL /etc/httpd/conf].d/whitelists/*


<b>Variables Directory</b>

This directory can contain all of the [!DNL \*.vars] files you create that contain variables you can consume in your configuration files

- *[!DNL /etc/httpd/conf].d/variables/*
...
- *[!DNL /etc/httpd/conf].dispatcher.d/*
...
- /etc/httpd/conf.dispatcher.d/enabled_farms/
   - When you're ready to use the farm file you have inside the available_farms folder symlink them using a relative path into the [!DNL enabled_farms] directory
...
- *[!DNL /etc/httpd/conf.dispatcher.d/cache/]*
...
- *[!DNL /etc/httpd/conf].dispatcher.d/clientheaders/*
...
- *[!DNL /etc/httpd/conf].dispatcher.d/filters/*
...
- *[!DNL /etc/httpd/conf].dispatcher.d/renders/*
...
- *[!DNL /etc/httpd/conf].dispatcher.d/vhosts/*
...
Enterprise linux was setup properly to handle this use case in a better way.  They give you areas in which you can extend or override the defaults they set for you.  Inside the base installation of [!DNL httpd] you'll find the file /etc/httpd/conf/httpd.conf, and it has syntax in it like:
...
As the perfect example when adding the dispatcher module to apache you'd create a module .so file in <b>[!DNL /etc/httpd/modules/]</b> and then include it by adding a file in <b>[!DNL /etc/httpd/conf].modules.d/02-dispatcher.conf</b> with the contents to load your module .so file
<br><br><br><br><br> <br><br><br><br>
...
Now we consume our module in our file <b>[!DNL /etc/httpd/conf].d/dispatcher_vhost.conf</b> which initializes our module and loads the inital module specific configuration file
<br><br><br><br><br> <br><br><br><br>
