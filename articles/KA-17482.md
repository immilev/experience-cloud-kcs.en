...
To manually generate a heap dump, run this command (jmap and jps can be found in the `[!DNL bin]` folder of your jdk home directory):
...
1. look up the pid of the java process you are generating a heap dump for.
   ...
   - In Windows this can be done by opening the task manager, press `Ctrl+Shift+Esc` then go to <b>[!UICONTROL View]</b> = <b>Select Columns</b> = <b>PID (Process Identifier)</b> or `jps -l`
