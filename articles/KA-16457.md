...
Running an Offline Compaction can fail with [!UICONTROL IllegalArgument] Exception when there are integrity issues of the repository. A Stack Trace similar to the one below can be found in the logs:
...
1. The segment has been removed by manual intervention (e.g. [!DNL rm] -rf /).
...
3. Remove all the identified corrupted nodes listed in the output of the last command using [!DNL rmNodes].groovy
   ...
   Note: When using [!DNL oak-run].jar version 1.6.13 and up, set --read-write JVM parameter if you run into an error such as:
...
4. Repeat step 3 for all nodes found in step 2.
   ...
   If still you see that the same paths are there in repository, then use the patched version of oak-run jar i.e. [!DNL oak-run-1.2.18-NPR-17596]
