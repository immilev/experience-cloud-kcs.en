...
Using [!DNL oak-run] jar 1.6.6 and above, run the following command:
...
In Linux, you could log in to each of the working [!DNL Adobe Experience Manager] instances and use a command like rsync to copy over any of the missing files that exist in them.  For example, by running it from the datastore directory on the server that has the missing files:
<br><br><br><br><br> <br><br><br><br>
...
2. Upload the oak-run to the [!DNL Adobe] Experience Manager server.
