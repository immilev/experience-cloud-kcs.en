---
title: Optimizing the [!DNL Dispatcher] cache
...
4. <b>Prime the cache </b>- Before deleting the [!DNL Dispatcher] cache, pull the [!DNL Dispatcher] off your load balancer, delete the cache, then run a web crawler tool to cache files on the dispatcher before putting it on the load balancer.
5. <b>Cache error pages</b> - Leverage the <b>[DispatcherPassError 1](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-install.html#ApacheWebServer) </b>([!DNL Apache] Web Server specific) directive to serve error pages such as 404s from the dispatcher cache.
6. <b>GZip compress all file types except for those that are pre-compressed </b>- In [!DNL Apache] Web Server, [mod_deflate](https://httpd.apache.org/docs/2.4/mod/mod_deflate.html) could be used, but make sure that <b>`Vary: User-Agent` </b>header<b> </b>isn't set.  In Microsoft IIS, use [Dynamic Compression](https://docs.microsoft.com/en-us/iis/configuration/system.webserver/httpcompression/).

   [!DNL Apache] configuration example (specifying only certain content types to avoid precompressed file types):
...
2. Install [!DNL Dispatcher] 4.1.11 or later.
